...
export function normalizeMonsterThreat(threat) {
    if (!threat) {
        return "Normal";
    }
    if (monsterThreatSchema.options.includes(threat)) {
        return threat;
    }
...
export const MONSTER_ATTRIBUTE_KEYS = [
    "quick",
    "vigilant",
    "accurate",
    "discreet",
...
function publishedLevel(value) {
    ...
    if (/\b(?:maestr[oa]|iii)\b/.test(normalized))
        return "maestro";
