...
function normalizeSkillLevel(level) {
    ...
    if (normalized === "novato" || normalized === "principiante")
        return "novato";
...
const ATAQUE_TRAICIONERO_ACTIONS = [
    capabilityAction("novato-ataque-traicionero", "Usar Ataque traicionero (Principiante)", "reaction", "Una vez por turno, al atacar con ventaja o sorpresa, puedes usar Discreto en vez de Diestro y causar +1D4 de daño adicional.", { rollAttribute: "discreto", damageFormula: "+1d4" }),
...
const ALIENTO_NEGRO_ACTIONS = [
    capabilityAction("novato-aliento-negro", "Lanzar Aliento negro (Principiante)", "combat", "Golpea a una criatura y tira 1D4 contra su corrupción total. Si el resultado es igual o inferior, la criatura se cura esa cantidad; si es superior, sufre esa cantidad como corrupción temporal.", { damageFormula: "1d4" }),
...
const CONFUSION_ACTIONS = [
    capabilityAction("novato-confusion", "Lanzar Confusión (Principiante)", "combat", "Supera [Tenaz←Tenaz] para confundir a un enemigo mientras mantengas la concentración. Cada turno tira 1D6: 1-2 queda inmóvil, 3-4 ataca al aliado más cercano, 5-6 ataca al enemigo más cercano.", { rollAttribute: "tenaz", damageFormula: "1d6" }),
...
const GOLPE_ESPECTRAL_ACTIONS = [
    capabilityAction("novato-golpe-espectral", "Lanzar Golpe espectral (Principiante)", "combat", "Imbuye tu arma cuerpo a cuerpo con una llama maligna hasta el final de la escena para causar +1D4 de daño adicional en cada impacto.", { damageFormula: "+1d4" }),
    capabilityAction("novato-golpe-psiquico-espectral", "Usar Golpe psíquico espectral (Principiante)", "reaction", "Combina tu ataque cuerpo a cuerpo con un golpe psíquico dirigido a la defensa del oponente y gana una segunda oportunidad de éxito en la prueba de ataque."),
...
const IMPOSICION_DE_MANOS_ACTIONS = [
    capabilityAction("novato-imposicion-de-manos", "Lanzar Imposición de manos (Principiante)", "combat", "Supera una tirada de Tenaz para curar 1D6 de Resistencia a un objetivo o a ti mismo.", { rollAttribute: "tenaz", damageFormula: "1d6" }),
...
const MURO_DE_LLAMAS_ACTIONS = [
    capabilityAction("novato-muro-de-llamas", "Lanzar Muro de llamas (Principiante)", "combat", "Supera una tirada de Tenaz para invocar un muro de fuego delante de ti. Cruzarlo inflige 1D12 de daño por fuego y puedes colocarlo sobre enemigos en línea frente a ti para herirlos automáticamente. Debes sostenerlo con una tirada de Tenaz cada turno.", { rollAttribute: "tenaz", damageFormula: "1d12" }),
...
const RAYO_NEGRO_ACTIONS = [
    capabilityAction("novato-rayo-negro", "Lanzar Rayo negro (Principiante)", "combat", "Supera una tirada de [Tenaz←Ágil] para impactar a un objetivo con un rayo negro. Si impacta, sufre 1D6 de daño ignorando armadura y queda atrapado hasta escapar con Tenaz o hasta que pierdas la concentración.", { rollAttribute: "tenaz", damageFormula: "1d6" }),
...
const SOMETER_VOLUNTAD_ACTIONS = [
    capabilityAction("novato-someter-voluntad", "Lanzar Someter voluntad (Principiante)", "combat", "Supera [Tenaz←Tenaz] para controlar a una criatura mientras mantengas la concentración. El objetivo solo puede realizar una acción por turno y no puede usar poderes ni habilidades activas.", { rollAttribute: "tenaz" }),
...
const ARCO_VELOZ_ACTIONS = [
    capabilityAction("novato-arco-veloz", "Usar Arco veloz (Principiante)", "combat", "Sacrifica tu acción de movimiento para disparar una segunda flecha este turno. Ambas flechas se resuelven por separado y pueden dirigirse al mismo objetivo o a dos distintos.", { rollAttribute: "diestro" }),
...
const ATAQUE_CON_DOS_ARMAS_ACTIONS = [
    capabilityAction("novato-ataque-con-dos-armas", "Usar Ataque con dos armas (Principiante)", "combat", "Haz dos ataques contra el mismo objetivo: el arma principal causa 1D8 y la secundaria 1D6. El enemigo debe defenderse por separado de cada ataque.", { damageFormula: "1d8/1d6" }),
...
const BERSERKER_ACTIONS = [
    capabilityAction("novato-berserker", "Entrar en frenesí (Principiante)", "free", "Entra en frenesí homicida para causar +1D6 de daño en combate cuerpo a cuerpo. Mientras dure, tu Defensa se calcula como si tuvieras Ágil 5.", { damageFormula: "+1d6" }),
    capabilityAction("novato-berserker-defensa", "Defender con Berserker (Principiante)", "reaction", "Mientras estés en frenesí, tu Defensa se resuelve como si tuvieras Ágil 5.", { rollAttribute: "agil", fixedTarget: 5 }),
...
const LUCHA_ACTIONS = [
    capabilityAction("novato-lucha", "Usar Lucha (Principiante)", "combat", "Haz un ataque normal y, si impactas, atrapa al oponente. Después puedes intentar lanzarlo o mantenerlo preso con [Fuerte←Fuerte]. El lanzamiento causa 1D4 ignorando armadura y lo deja boca arriba.", { rollAttribute: "fuerte", damageFormula: "1d4" }),
...
const PIROTECNIA_ACTIONS = [
    capabilityAction("novato-pirotecnia", "Usar polvo cegador (Principiante)", "combat", "Haz una tirada de [Diestro←Ágil] para deslumbrar a un oponente en cuerpo a cuerpo. Si impactas, causas 1D4 de daño ignorando armadura y lo dejas cegado durante 1D4 turnos.", { rollAttribute: "diestro", damageFormula: "1d4" }),
...
const PUNO_DE_FLECHA_ACTIONS = [
    capabilityAction("novato-puno-de-flecha", "Usar Puño de flecha (Principiante)", "reaction", "Responde a un ataque cuerpo a cuerpo clavando una flecha o virote al rival con una tirada de ataque normal. Si impactas, causas 1D6 y aplicas las cualidades del proyectil.", { damageFormula: "1d6" }),
...
const RECUPERACION_ACTIONS = [
    capabilityAction("novato-recuperacion", "Usar Recuperación (Principiante)", "combat", "Supera una tirada de Tenaz para recuperar 1D4 de Resistencia. Solo puedes tener un éxito por día.", { rollAttribute: "tenaz", damageFormula: "1d4" }),
...
const ACROBATA_ACTIONS = [
    capabilityAction("novato-acrobata", "Usar Acróbata (Principiante)", "combat", "Supera una tirada de Ágil para evitar los ataques gratuitos provocados al retirarte de un combate cuerpo a cuerpo o al pasar junto a un enemigo.", { rollAttribute: "agil" }),
...
const ARMAS_DE_PRESA_ACTIONS = [
    capabilityAction("novato-armas-de-presa", "Usar Armas de presa (Principiante)", "combat", "Al atacar con un arma con la cualidad Presa, ganas una segunda oportunidad para inmovilizar al objetivo."),
...
const COMBATE_SANGRIENTO_ACTIONS = [
    capabilityAction("novato-combate-sangriento", "Usar Combate sangriento (Principiante)", "reaction", "Cuando tu Resistencia cae hasta la mitad, obtienes una segunda oportunidad en todas tus tiradas de ataque cuerpo a cuerpo."),
...
const DANZA_DE_BATALLA_ACTIONS = [
    capabilityAction("novato-danza-de-batalla", "Usar Danza de batalla (Principiante)", "reaction", "Divide tu acción de movimiento para hacer una parte antes de la acción de combate y otra después."),
...
const DISPARO_MAGISTRAL_ACTIONS = [
    capabilityAction("novato-disparo-magistral", "Usar Disparo magistral (Principiante)", "combat", "Invierte turno completo en apuntar para desarmar, clavar o cegar al objetivo. Si impactas e infliges daño, además aplica el efecto elegido y el objetivo pierde una acción de combate.", { rollAttribute: "diestro" }),
...
const GOLPE_BAJO_ACTIONS = [
    capabilityAction("novato-golpe-bajo", "Usar Golpe bajo (Principiante)", "combat", "Haz un golpe bajo que causa 1D6 de daño y, si hiere al oponente, te concede un ataque gratuito contra él.", { damageFormula: "1d6" }),
...
const GUARDAESPALDAS_ACTIONS = [
    capabilityAction("novato-guardaespaldas", "Usar Guardaespaldas (Principiante)", "reaction", "Supera una tirada de Tenaz para recibir tú un golpe dirigido contra un aliado. No puedes defenderte de ese ataque.", { rollAttribute: "tenaz" }),
...
const JINETE_ACTIONS = [
    capabilityAction("novato-jinete", "Cargar con Jinete (Principiante)", "reaction", "Si tu montura se mueve antes de atacar, puedes causar +1D6 de daño adicional en un ataque cuerpo a cuerpo.", { damageFormula: "+1d6" }),
...
const MEDICUS_ACTIONS = [
    capabilityAction("novato-medicus", "Usar Medicus (Principiante)", "combat", "Haz una tirada de Inteligente para curar 1D4 de Resistencia a un objetivo, o 1D6 si usas hierbas curativas. Solo una vez al día por paciente.", { rollAttribute: "inteligente", damageFormula: "1d4" }),
...
const OPORTUNISTA_ACTIONS = [
    capabilityAction("novato-oportunista", "Usar Oportunista (Principiante)", "reaction", "Obtienes una segunda oportunidad para acertar ataques gratuitos contra un oponente que se retira del combate cuerpo a cuerpo."),
...
const REFLEJOS_RAPIDOS_ACTIONS = [
    capabilityAction("novato-reflejos-rapidos", "Usar Reflejos rápidos (Principiante)", "reaction", "Frente a efectos de área o similares que infligen daño completo o mitad, pasas a sufrir mitad o ningún daño respectivamente."),
...
const TRAMPERO_ACTIONS = [
    capabilityAction("novato-trampero", "Usar Trampero (Principiante)", "combat", "Haz una tirada de Inteligente para desplegar o desarmar una trampa mecánica. También puedes construir una trampa improvisada que causa 1D6 de daño.", { rollAttribute: "inteligente", damageFormula: "1d6" }),
...
const VENENOS_ACTIONS = [
    capabilityAction("novato-venenos", "Aplicar veneno (Principiante)", "free", "Aplica una dosis de veneno a un arma para un solo golpe. La víctima queda envenenada si superas [Inteligente←Fuerte].", { rollAttribute: "inteligente" }),
...
const VERSADO_EN_CRIATURAS_ACTIONS = [
    capabilityAction("novato-versado-en-criaturas", "Analizar criatura (Principiante)", "free", "Haz una tirada de Inteligente para reconocer o recordar puntos débiles y fuertes de un monstruo.", { rollAttribute: "inteligente" }),
...
const CANALIZACION_ACTIONS = [
    capabilityAction("novato-canalizacion", "Usar Canalización (Principiante)", "reaction", "Recibe sobre ti la corrupción temporal que iba a sufrir otra criatura a la vista."),
...
const ESTRANGULADOR_ACTIONS = [
    capabilityAction("novato-estrangulador", "Usar Estrangulador (Principiante)", "combat", "Si tienes Ventaja, realiza un ataque que, si impacta, causa 1D6 por turno ignorando armadura e inmoviliza al objetivo hasta que falles la presa.", { damageFormula: "1d6" }),
...
const EXPERTO_EN_ASEDIOS_ACTIONS = [
    capabilityAction("novato-experto-en-asedios", "Usar arma de asedio o granada (Principiante)", "combat", "Maneja balistas y granadas alquímicas de forma segura, sin riesgo de detonación accidental."),
...
const MANO_VELOZ_ACTIONS = [
    capabilityAction("novato-mano-veloz", "Usar Mano veloz (Principiante)", "free", "Supera una tirada de Ágil para desenvainar y empuñar un arma, o para recargar una ballesta, como acción gratuita.", { rollAttribute: "agil" }),
...
const MARTILLO_ARIETE_ACTIONS = [
    capabilityAction("novato-martillo-ariete", "Usar Martillo ariete (Principiante)", "reaction", "Si un escudo desvía tu ataque de martillo, supera [Fuerte←Ágil] para romper un escudo de madera o arrancar uno metálico, causando además 1D6 al portador.", { rollAttribute: "fuerte", damageFormula: "1d6" }),
...
const MAESTRO_DEL_HACHA_ACTIONS = [
    capabilityAction("novato-maestro-del-hacha", "Usar Maestro del hacha (Principiante)", "combat", "Golpea con el extremo corto del hacha para causar 1D6 de daño y, si superas [Diestro←Tenaz], aturdir al objetivo y ganar un ataque gratuito inmediato.", { rollAttribute: "diestro", damageFormula: "1d6" }),
...
const OJO_MISTICO_ACTIONS = [
    capabilityAction("novato-ojo-mistico", "Usar Ojo místico (Principiante)", "free", "Supera [Atento←Discreto] para ver la Sombra dominante de una criatura, lugar u objeto. Cada intento te causa 1D4 de corrupción temporal.", { rollAttribute: "atento" }),
...
const SIMBOLISMO_ACTIONS = [
    capabilityAction("novato-simbolismo", "Activar símbolo preparado (Principiante)", "combat", "Activa un símbolo preparado a la vista o en contacto pronunciando su frase de activación."),
...
const TATUAJE_RUNICO_ACTIONS = [
    capabilityAction("novato-tatuaje-runico", "Activar Tatuaje rúnico (Principiante)", "reaction", "Activa la runa antes de tirar protección para obtener +1D4 a la armadura contra ese ataque, al coste de 1 punto de corrupción temporal.", { damageFormula: "+1d4" }),
