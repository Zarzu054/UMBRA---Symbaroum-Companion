import type { ActorCapabilitySelection } from "./actorCreation.js";
...
export type ProfessionBenefit = {
  name: string;
  kind: "habilidad" | "poder_mistico" | "ritual" | "rasgo_monstruoso";
  upgradesFrom?: string;
...
export type ProfessionDefinition = {
  id: string;
  name: string;
  archetype: "Cazador" | "Guerrero" | "Místico" | "Maleante";
  page: number;
  summary: string;
  ...
  otherRequirement?: {
    ...
    cultures?: string[];
    blessing?: string;
    maximumPermanentCorruptionAtAdmission?: number;
  };
  benefits: ProfessionBenefit[];
};

const req = (id: string, label: string, ...anyOfNames: string[]): ProfessionRequirement => ({ id, label, anyOfNames });
const ritualReq = (id: string, label: string): ProfessionRequirement => ({ id, label, anyRitual: true });

export const SYMBAROUM_PROFESSIONS: ProfessionDefinition[] = [
  {
...
export type ProfessionEvaluationContext = {
  race: string;
  culture: string;
...
export type ProfessionRequirementResult = {
  id: string;
  label: string;
  met: boolean;
  matchedNames: string[];
...
export type ProfessionEligibility = {
  professionId: string;
  eligible: boolean;
  requirementsMet: boolean;
  masterRequirementMet: boolean;
  otherRequirementMet: boolean;
  requirementResults: ProfessionRequirementResult[];
  unmetRequirements: string[];
};

export function normalizeProfessionText(value: string): string {
...
export function getProfessionByName(name: string): ProfessionDefinition | undefined {
...
function isMasterCapability(capability: Pick<ActorCapabilitySelection, "kind" | "level">): boolean {
  return capability.kind !== "ritual" && capability.level === "maestro";
}

export function evaluateProfession(
  definitionOrId: ProfessionDefinition | string,
  context: ProfessionEvaluationContext,
  options: { includeAdmissionOnly?: boolean } = {}
): ProfessionEligibility {
  ...
  const requirementResults = definition.requirements.map((requirement) => {
    const matches = requirement.anyRitual
      ...
      : (requirement.anyOfNames ?? []).map((name) => byName.get(normalizeProfessionText(name))).filter((entry): entry is Pick<ActorCapabilitySelection, "name" | "kind" | "level"> => Boolean(entry));
  ...
  if (other?.races?.length) otherRequirementMet &&= other.races.some((value) => normalizeProfessionText(value) === normalizeProfessionText(context.race));
  if (other?.cultures?.length) otherRequirementMet &&= other.cultures.some((value) => normalizeProfessionText(value) === normalizeProfessionText(context.culture));
  if (other?.blessing) otherRequirementMet &&= context.blessings.some((value) => normalizeProfessionText(value) === normalizeProfessionText(other.blessing!));
  ...
  if (!masterRequirementMet) unmetRequirements.push("Al menos una capacidad requerida a nivel maestro");
  if (!otherRequirementMet && other) unmetRequirements.push(other.label);
...
const benefitAccess = new Map<string, string[]>();
...
export function getBenefitProfessionIds(name: string): string[] {
  return benefitAccess.get(normalizeProfessionText(name)) ?? [];
}

export function isProfessionExclusiveBenefit(name: string): boolean {
  return getBenefitProfessionIds(name).length > 0;
}

export function getHigherRitualBase(name: string): string | undefined {
  for (const profession of SYMBAROUM_PROFESSIONS) {
    const benefit = profession.benefits.find((entry) => normalizeProfessionText(entry.name) === normalizeProfessionText(name));
    if (benefit?.upgradesFrom) return benefit.upgradesFrom;
  }
  return undefined;
}
