...
export function getActorChallengeFromXp(xp) {
...
export function getRatedCapabilityXp(level) {
    if (level === "maestro")
        return 60;
    if (level === "adepto")
        return 30;
...
export function getActorCapabilityXpDelta(entry) {
    if (entry.kind === "carga")
        return -5;
    if (entry.kind === "rasgo_personaje")
        return 0;
    if (entry.kind === "bendicion")
        return entry.origin === "racial" ? 0 : 5;
    if (entry.kind === "ritual")
        return 10;
    return getRatedCapabilityXp(entry.level);
}
export function getActorSpentXp(entries) {
    return entries.reduce((total, entry) => total + Math.max(0, getActorCapabilityXpDelta(entry)), 0);
}
export function getActorBurdenBonus(entries) {
    return entries.reduce((total, entry) => total + (entry.kind === "carga" ? 5 : 0), 0);
}
function normalizeCapabilityName(value) {
    return value.normalize("NFD").replace(/[\u0300-\u036f]/g, "").trim().toLowerCase();
}
export function isExceptionalAttributeSelection(entry) {
    return normalizeCapabilityName(entry.name) === "atributo excepcional";
}
export function getExceptionalAttributeBonus(entry) {
    if (!isExceptionalAttributeSelection(entry) || !entry.attributeKey)
        return 0;
    if (entry.level === "maestro")
        return 3;
    if (entry.level === "adepto")
        return 2;
    return 1;
}
export function getExceptionalAttributeBonuses(entries) {
    return entries.reduce((bonuses, entry) => {
        const bonus = getExceptionalAttributeBonus(entry);
        if (entry.attributeKey && bonus > 0)
            bonuses[entry.attributeKey] = (bonuses[entry.attributeKey] ?? 0) + bonus;
        return bonuses;
    }, {});
}
export function removeExceptionalAttributeBonuses(finalValues, entries) {
    const bonuses = getExceptionalAttributeBonuses(entries);
    return Object.fromEntries(Object.entries(finalValues).map(([key, value]) => [key, Number(value) - (bonuses[key] ?? 0)]));
}
export function applyExceptionalAttributeBonuses(baseValues, entries) {
    const bonuses = getExceptionalAttributeBonuses(entries);
    return Object.fromEntries(Object.entries(baseValues).map(([key, value]) => [key, Number(value) + (bonuses[key] ?? 0)]));
}
export function synchronizeExceptionalAttributes(finalValues, previousEntries, nextEntries) {
    return applyExceptionalAttributeBonuses(removeExceptionalAttributeBonuses(finalValues, previousEntries), nextEntries);
}
export function validateExceptionalAttributeSelections(entries, allowedKeys) {
    const exceptional = entries.filter(isExceptionalAttributeSelection);
    const errors = [];
    const selected = new Set();
    for (const entry of exceptional) {
        if (!entry.attributeKey) {
            if (entry.origin !== "legado")
                errors.push("Cada adquisición de Atributo excepcional debe indicar un atributo.");
            continue;
        }
        if (!allowedKeys.includes(entry.attributeKey))
            errors.push(`El atributo ${entry.attributeKey} no es válido para Atributo excepcional.`);
        if (selected.has(entry.attributeKey))
            errors.push("Atributo excepcional solo puede adquirirse una vez para cada atributo.");
 ...
 */
export function averageDiceFormula(formula) {
    ...
    for (const rawTerm of terms) {
        ...
        if (dice) {
            ...
            if (!Number.isFinite(count) || count < 1 || !Number.isFinite(sides) || sides < 2)
                return null;
...
export function validateCreationAttributes(values) {
    ...
    if (total !== 80)
        errors.push("La suma de atributos debe ser exactamente 80.");
    if (numbers.filter((value) => value === 15).length > 1)
        errors.push("Solo un atributo puede tener valor 15.");
