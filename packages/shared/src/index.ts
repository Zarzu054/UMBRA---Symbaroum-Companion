...
export const skillLevelSchema = z.preprocess(
  ...
  (value) => value === "novato" ? "principiante" : value,
  canonicalSkillLevelSchema
);
...
export type CharacterActionCategory = z.infer<typeof characterActionCategorySchema>;
 ...
 */
export function formatSkillLevelLabel(level: SkillLevel | "principiante" | string | null | undefined): string {
...
export function replaceLegacySkillLevelTerminology(value: string): string {
  return value.replace(/\bnovat(?:o|a)(s?)\b/giu, (_match, plural: string) => plural ? "Principiantes" : "Principiante");
}
...
function parseMonsterTraitLevel(value: string): "principiante" | "adepto" | "maestro" {
  ...
  for (const item of sheet.inventoryItems) {
    ...
    for (const action of item.grantedActions ?? []) {
      ...
      actions.push({
        ...
        sourceName: item.name,
        cost: action.cost,
        categories: buildCanonicalActionCategories(
          sourceType,
          action.cost,
          action.categories,
          action.rolls?.some((roll) => roll.kind === "attack") || Boolean(action.rollAttribute && action.damageFormula)
        ),
...
function buildCanonicalActionCategories(
  sourceType: "weapon" | "ability" | "power" | "ritual" | "artifact" | "utility",
  cost: z.infer<typeof actionCostSchema>,
  provided: z.infer<typeof characterActionCategorySchema>[] = [],
  isAttack = false
): z.infer<typeof characterActionCategorySchema>[] {
  const categories = new Set<z.infer<typeof characterActionCategorySchema>>(provided);
  categories.add(cost);
  if (isAttack) categories.add("attack");
  if (sourceType === "power" || sourceType === "ritual") categories.add("powers");
  if (sourceType === "artifact") categories.add("artifacts");
  return [...categories];
}
...
function inferCanonicalFallbackAction(
  ...
  return {
    ...
    categories: buildCanonicalActionCategories(
      ...
      cost,
      [],
      /\b(ataque|atacar|dispar|golpe|embest|placaje|lanzar)\b/i.test(trimmedText)
    ),
...
function characterHasCombatCapability(sheet: CharacterSheet, name: string, minimumLevel: "principiante" | "adepto" | "maestro"): boolean {
...
function resolveCharacterInitiativeAttribute(sheet: CharacterSheet): number {
  ...
  if (characterHasCombatCapability(sheet, "Táctico", "principiante")) values.push(sheet.atributos.inteligente);
...
export type CharacterActionDefinition = {
  ...
  linkedItemId?: string;
  cost: ActionCost;
  categories?: CharacterActionCategory[];
  requiredLevel?: SkillLevel;
  rollAttribute?: AttributeKey;
  opponentAttribute?: AttributeKey;
  fixedTarget?: number;
  ...
  artifactAbilityId?: string;
  disabledReason?: string;
  rolls?: Array<{
    id: string;
    kind: "check" | "attack" | "damage" | "armor" | "healing" | "custom";
    label: string;
    formula: string;
    actorAttribute?: AttributeKey;
    opponentAttribute?: AttributeKey;
    fixedTarget?: number;
  }>;
};
...
export type CampaignItemDefinition = z.infer<typeof campaignItemDefinitionSchema>;
...
export type AssignCampaignItemOwnerInput = z.infer<typeof assignCampaignItemOwnerSchema>;

export type CampaignItemTemplate = {
  id: string;
  campaignId: string;
  kind: z.infer<typeof campaignItemKindSchema>;
  definition: CampaignItemDefinition;
  isUnique: boolean;
  ownerType: z.infer<typeof campaignItemOwnerTypeSchema> | null;
  ownerId: string | null;
