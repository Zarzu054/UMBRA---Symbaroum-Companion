...
export function projectMysticArtifactsIntoSheet(input, artifacts) {
    const unmanagedItems = input.inventoryItems.filter((item) => !item.managedArtifactId);
    const managedItems = artifacts.map((artifact) => {
    ...
    const preserveSlot = (value) => value.startsWith("managed-artifact:") && !validManagedIds.has(value) ? "" : value;
    return {
        ...input,
...
export function stripManagedMysticArtifactsFromSheet(input) {
    const isManagedId = (value) => value.startsWith("managed-artifact:");
    return {
        ...input,
...
export function preserveLegacyMysticArtifacts(current, requested) {
