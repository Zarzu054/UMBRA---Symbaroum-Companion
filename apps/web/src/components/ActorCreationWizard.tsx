...
function buildLegacySelections(sheet: CharacterSheet): ActorCapabilitySelection[] {
  ...
  return [
    ...
    ...sheet.rituales.map((entry) => ({ catalogId: `ritual-${normalize(entry.nombre).replace(/\s+/g, "-")}`, name: entry.nombre, kind: "ritual" as const, level: "principiante" as const, origin: "legado" as const, source: entry.fuente, page: entry.pagina })),
...
function updateLegacyCollections(sheet: CharacterSheet, selections: ActorCapabilitySelection[]): CharacterSheet {
  ...
  const toRated = (entry: ActorCapabilitySelection) => {
    ...
    return {
      ...
      nivel: entry.level ?? "principiante" as SkillLevel,
      fuente: entry.source,
      pagina: entry.page,
...
export function CharacterCreationWizard({ controller, onCancel }: { controller: CharacterController; onCancel: () => void }) {
  ...
  const experience = getCharacterExperienceSummary(
    { ...sheet, capabilitySelections: selections },
    { includeBurdenBonus: !controller.isEditing }
  );
