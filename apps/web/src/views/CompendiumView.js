...
import { ALL_ENTRIES, TYPE_LABELS, canonicalizeCompendiumSourceName, getCompendiumSourcePdfUrl } from "../models/compendiumEntries";
...
export function CompendiumView({ onBackToCharacters, initialEntryId = null, initialQuery = "", initialSourceFilter = "all", focusToken = 0 }) {
    ...
    const sources = useMemo(() => ["all", ...new Set(ALL_ENTRIES.map((entry) => canonicalizeCompendiumSourceName(entry.fuente)))], []);
    useEffect(() => {
        ...
        setTypeFilter("all");
        setSourceFilter(initialSourceFilter !== "all"
