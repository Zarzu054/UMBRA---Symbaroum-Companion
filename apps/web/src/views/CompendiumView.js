...
export function CompendiumView({ onBackToCharacters, ensureAccessToken, initialEntryId = null, initialQuery = "", initialSourceFilter = "all", initialTypeFilter = "all", initialRuleCategory = "all", initialBrowseMode = "type", focusToken = 0 }) {
    ...
    function renderLibraryEntries(entries, emptyText) {
        ...
        return (_jsx("div", { className: "compendium-shelf-list", children: entries.map((entry) => (_jsxs("button", { type: "button", className: `compendium-shelf-entry app-card-accent app-card-accent--${entry.tipo}${entry.ruleCategory ? ` rule-category--${entry.ruleCategory}` : ""}`, onClick: () => {
                    closeLibraryModal(false);
                    openEntry(entry.id, libraryModalTriggerRef.current ?? undefined);
                }, children: [_jsx("span", { className: "compendium-shelf-entry-title", children: entry.nombre }), _jsxs("span", { children: [getEntryTypeLabel(entry), " \u00B7 ", canonicalizeCompendiumSourceName(entry.fuente)] })] }, entry.id))) }));
    }
