...
import { acceptCampaignCharacterLinkRequest, acceptCampaignInvitation, createCampaign, createCampaignReference, deleteCampaignReference, decideProfessionRequest, dismissCampaignCharacterLinkRequest, fetchCampaigns, fetchCampaignCharacterLinkRequests, fetchCampaignInvitations, removeCampaignMember, dismissCampaignInvitation, requestCampaignCharacterLink, sendCampaignInvitation, unlinkCampaignCharacter, grantCampaignExperience, updateCampaignCharacterSheet, updateCampaign, updateCampaignReference } from "../services/campaignService";
...
function replaceCampaignHash(campaignId, sheetId, section, invitationId = null, characterRequestId = null) {
...
export function CampaignDashboardView({ user, ensureAccessToken }) {
    ...
    const [characterLinkRequests, setCharacterLinkRequests] = useState([]);
    const [focusedInvitationId, setFocusedInvitationId] = useState(initialHash.invitationId);
    const [focusedCharacterLinkRequestId, setFocusedCharacterLinkRequestId] = useState(initialHash.characterRequestId);
    ...
    const [professionRequestsCharacterId, setProfessionRequestsCharacterId] = useState(null);
    ...
    const [campaignObjectTab, setCampaignObjectTab] = useState("weapon");
    ...
    const professionRequestsCharacter = useMemo(() => selectedCampaign?.characters.find((entry) => entry.characterId === professionRequestsCharacterId) ?? null, [professionRequestsCharacterId, selectedCampaign]);
    const selectedProfessionRequests = useMemo(() => professionRequestsCharacter
    ...
    const focusedCharacterLinkRequest = useMemo(() => characterLinkRequests.find((request) => request.id === focusedCharacterLinkRequestId) ?? null, [characterLinkRequests, focusedCharacterLinkRequestId]);
    ...
    const linkableCharacters = useMemo(() => {
        const requestedCharacterIds = new Set((selectedCampaign?.pendingCharacterLinkRequests ?? []).map((request) => request.characterId));
        return (selectedCampaign?.availableCharacters ?? []).filter((entry) => !entry.linked && !requestedCharacterIds.has(entry.characterId) && (isDirector || entry.ownerId === user.id));
    ...
    useEffect(() => {
        replaceCampaignHash(selectedCampaignId, selectedSheetId, selectedCampaignId ? activeSection : null, focusedInvitationId, focusedCharacterLinkRequestId);
    ...
    async function handleRequestCharacterLink() {
    ...
    async function handleAcceptCharacterLinkRequest(requestId) {
        setFormError(null);
        setIsSaving(true);
        try {
            const token = await ensureAccessToken();
    ...
    async function handleDismissCharacterLinkRequest(requestId, campaignId) {
        setFormError(null);
        setIsSaving(true);
        try {
            const token = await ensureAccessToken();
    ...
    return (_jsxs("main", { className: "campaign-dashboard", children: [!selectedCampaign ? (_jsxs(_Fragment, { children: [_jsx("header", { className: "panel module-sticky-header module-sticky-header--single-row campaign-module-header", children: _jsxs("div", { className: "row-actions", children: [_jsxs("div", { children: [_jsx("h1", { children: "Campa\u00F1as" }), _jsx("p", { className: "section-help", children: "Notas compartidas, notas del DJ y personajes vinculados." })] }), _jsxs("div", { className: "toolbar", children: [isDirector ? (_jsx("button", { type: "button", onClick: () => {
                                            ...
                                            : "Aun no hay notas privadas registradas." })) : null] })] })) : null, activeSection === "sharedNotes" ? (_jsxs("section", { className: "panel", children: [_jsxs("div", { className: "row-actions campaign-notes-heading", children: [_jsxs("div", { className: "campaign-notes-copy", children: [_jsx("h3", { children: "Notas compartidas" }), _jsx("p", { className: "section-help", children: "Entradas en Markdown visibles para toda la campa\u00F1a, con busqueda por titulo y enlaces a la wiki detectados dentro de cada nota." })] }), _jsxs("div", { className: "inline-row campaign-inline-form campaign-notes-controls", role: "group", "aria-label": "Controles de notas compartidas", children: [_jsxs("label", { className: "field", children: [_jsx("span", { children: "Buscar por titulo" }), _jsx("input", { value: sharedNoteSearch, onChange: (event) => setSharedNoteSearch(event.target.value), placeholder: "Nombre de la nota" })] }), _jsxs("label", { className: "field", children: [_jsx("span", { children: "Ordenar" }), _jsxs("select", { value: sharedNoteSort, onChange: (event) => setSharedNoteSort(event.target.value), children: [_jsx("option", { value: "updated_desc", children: "Mas recientes" }), _jsx("option", { value: "updated_asc", children: "Mas antiguas" }), _jsx("option", { value: "title_asc", children: "Titulo A-Z" }), _jsx("option", { value: "title_desc", children: "Titulo Z-A" })] })] }), _jsx("button", { type: "button", disabled: isSaving, onClick: () => {
                                                    setSharedNoteError(null);
                                        ...
                                        return (_jsxs("article", { className: "card character-record-card campaign-character-card", "aria-label": `Personaje ${entry.name}`, children: [_jsxs("div", { className: "character-record-card-head", children: [_jsx("div", { className: "character-record-card-portrait", "aria-hidden": "true", children: _jsx("span", { children: initials }) }), _jsxs("div", { className: "character-record-card-copy", children: [_jsx("h3", { children: entry.name }), _jsx("p", { children: entry.ownerEmail }), _jsxs("p", { className: "campaign-character-xp-summary", children: [_jsxs("strong", { children: [availableExperience, " PX disponibles"] }), _jsxs("span", { children: [entry.experienceSpent, " gastadas de ", entry.experienceTotal] })] }), entry.sheetLoadError ? _jsx("span", { className: "error-text", children: "La ficha necesita reparaci\u00F3n, pero la campa\u00F1a sigue disponible." }) : null] })] }), _jsxs("small", { className: "character-record-card-updated", children: ["Actualizada ", formatDate(entry.updatedAt)] }), _jsxs("div", { className: "card-actions", children: [isDirector && entry.sheet ? (_jsx("button", { type: "button", className: "character-record-primary-action", onClick: () => {
                                                                setSelectedSheetId(entry.id);
                                                                setCampaignCharacterView("sheet");
                                                            }, children: "Abrir hoja" })) : null, isDirector ? (_jsx("button", { type: "button", className: "campaign-character-grant-action", disabled: isSaving, onClick: () => {
                                                                setExperienceGrantError(null);
                                                            ...
                                                            }, children: "Conceder PX" })) : null, isDirector && entry.sheet ? (_jsx("button", { type: "button", className: "subtle-button character-record-builder-action", onClick: () => {
                                                                setSelectedSheetId(entry.id);
                                                                setCampaignCharacterView("builder");
                                                            }, children: "Constructor" })) : null, !isDirector && isMobile ? (_jsxs(_Fragment, { children: [isCharacterOwner ? (_jsxs("button", { type: "button", className: "character-history-button campaign-character-direct-action", "aria-label": `Historial de cambios de ${entry.name}`, onClick: () => setChangeLogCharacterId(entry.characterId), children: ["Historial de cambios", unreadChangeCount > 0 ? _jsx("span", { className: "character-history-badge", "aria-label": `${unreadChangeCount} cambios sin leer`, children: unreadChangeCount }) : null] })) : (_jsx("button", { type: "button", className: "campaign-character-direct-action", onClick: () => setExperienceHistoryCharacterId(entry.characterId), children: "Historial de PX" })), isCharacterOwner ? (_jsxs("details", { className: "character-record-actions-menu", children: [_jsx("summary", { children: "M\u00E1s acciones" }), _jsxs("div", { className: "character-record-secondary-actions", children: [_jsx("button", { type: "button", onClick: () => setExperienceHistoryCharacterId(entry.characterId), children: "Historial de PX" }), _jsx("button", { type: "button", className: "danger", disabled: isSaving, onClick: () => {
                                                                                        setFormError(null);
                                                                                        setPendingUnlinkCharacter(entry);
                                                                                    }, children: "Desvincular" })] })] })) : null] })) : (_jsxs("details", { className: "character-record-actions-menu", children: [_jsxs("summary", { children: [_jsx("span", { children: "M\u00E1s acciones" }), unreadChangeCount > 0 ? (_jsx("span", { className: "character-history-badge", "aria-label": `${unreadChangeCount} cambios sin leer`, children: unreadChangeCount })) : null] }), _jsxs("div", { className: "character-record-secondary-actions", children: [isDirector ? (_jsxs("button", { type: "button", className: "campaign-character-profession-action", "aria-label": `Solicitudes de profesión de ${entry.name}: ${pendingProfessionRequestCount} ${pendingProfessionRequestCount === 1 ? "pendiente" : "pendientes"}`, onClick: () => {
                                                                                setFormError(null);
                                                                                setProfessionRequestsCharacterId(entry.characterId);
                                                                            }, children: [_jsx("span", { children: "Solicitudes de profesi\u00F3n" }), _jsx("strong", { children: pendingProfessionRequestCount })] })) : null, canViewChangeLog ? (_jsxs("button", { type: "button", className: "character-history-button", "aria-label": `Historial de cambios de ${entry.name}`, onClick: () => setChangeLogCharacterId(entry.characterId), children: ["Historial de cambios", unreadChangeCount > 0 ? _jsx("span", { className: "character-history-badge", "aria-label": `${unreadChangeCount} cambios sin leer`, children: unreadChangeCount }) : null] })) : null, _jsx("button", { type: "button", onClick: () => setExperienceHistoryCharacterId(entry.characterId), children: "Historial de PX" }), canManageLink ? (_jsx("button", { type: "button", className: "danger", disabled: isSaving, onClick: () => {
                                                                                setFormError(null);
                                                                                setPendingUnlinkCharacter(entry);
                            ...
                            setCampaignCharacterView("sheet");
                        } })) : null, isDirector && activeSection === "artifacts" ? (_jsxs("section", { className: "panel", children: [_jsx("div", { className: "toolbar campaign-object-tabs", role: "tablist", "aria-label": "Tipos de objetos de campa\u00F1a", children: [[
                                        "weapon", "Armas"
                                    ], ["armor", "Armaduras"], ["item", "Objetos"], ["artifacts", "Artefactos místicos"]].map(([tab, label]) => (_jsx("button", { type: "button", role: "tab", "aria-selected": campaignObjectTab === tab, className: campaignObjectTab === tab ? "is-active" : "", onClick: () => setCampaignObjectTab(tab), children: label }, tab))) }), campaignObjectTab !== "artifacts" ? (_jsx(CampaignItemManager, { campaign: selectedCampaign, kind: campaignObjectTab, ensureAccessToken: ensureAccessToken, onRefresh: refresh })) : (_jsxs(_Fragment, { children: [_jsxs("div", { className: "row-actions", children: [_jsxs("div", { children: [_jsx("h3", { children: "Artefactos m\u00EDsticos" }), _jsx("p", { className: "section-help", children: "Solo se muestran los artefactos que el DJ ha incluido en esta campa\u00F1a." })] }), _jsx("button", { type: "button", disabled: isSaving, onClick: () => {
                                                    setArtifactError(null);
                    ...
                    setSelectedSheetId(null);
                                    ...
                                    await refresh();
                                    return updated;
                                }, onOpenBuilder: () => setCampaignCharacterView("builder"), onSave: (sheet) => handleSaveCampaignCharacter(campaignSheetModalEntry, sheet, "sheet"), onUseArtifactAbility: async (artifactId, abilityId) => {
                                    const token = await ensureAccessToken();
                                    await useMysticArtifactAbility(artifactId, abilityId, token);
                                    await refresh();
            ...
            })() : null, experienceHistoryCharacter ? (_jsx("section", { className: "modal-backdrop", onClick: () => setExperienceHistoryCharacterId(null), children: _jsxs("div", { className: "panel modal-panel campaign-experience-history-modal", role: "dialog", "aria-modal": "true", "aria-labelledby": "campaign-experience-history-title", onClick: (event) => event.stopPropagation(), children: [_jsxs("header", { className: "row-actions", children: [_jsxs("div", { children: [_jsxs("h2", { id: "campaign-experience-history-title", children: ["Historial de PX de ", experienceHistoryCharacter.name] }), _jsxs("p", { className: "section-help", children: ["Total: ", experienceHistoryCharacter.experienceTotal, " \u00B7 Gastada: ", experienceHistoryCharacter.experienceSpent, " \u00B7 Disponible: ", Math.max(0, experienceHistoryCharacter.experienceTotal - experienceHistoryCharacter.experienceSpent)] })] }), _jsx("button", { type: "button", className: "subtle-button", onClick: () => setExperienceHistoryCharacterId(null), children: "Cerrar" })] }), _jsx("div", { className: "campaign-experience-history-body", children: selectedExperienceHistory.length > 0 ? (_jsx("div", { className: "campaign-character-experience-list", children: selectedExperienceHistory.map((logEntry) => (_jsxs("article", { className: "campaign-character-experience-entry", children: [_jsxs("strong", { children: ["+", logEntry.amount, " PX"] }), _jsx("span", { children: logEntry.reason }), _jsxs("span", { children: [formatDate(logEntry.createdAt), " \u00B7 ", logEntry.grantedByEmail] })] }, logEntry.id))) })) : _jsx("p", { className: "section-help", children: "Todav\u00EDa no hay experiencia concedida a este personaje." }) })] }) })) : null, isDirector && professionRequestsCharacter ? (_jsx("section", { className: "modal-backdrop", onClick: () => {
                    setFormError(null);
                    setProfessionRequestsCharacterId(null);
                }, children: _jsxs("div", { className: "panel modal-panel profession-request-modal", role: "dialog", "aria-modal": "true", "aria-labelledby": "profession-requests-title", onClick: (event) => event.stopPropagation(), children: [_jsxs("div", { className: "row-actions", children: [_jsxs("div", { children: [_jsxs("h3", { id: "profession-requests-title", children: ["Solicitudes de profesiones de ", professionRequestsCharacter.name] }), _jsx("p", { className: "section-help", children: "Solo se muestran las solicitudes de esta ficha. Los requisitos se comprobar\u00E1n de nuevo al aprobar." })] }), _jsx("button", { type: "button", className: "subtle-button", onClick: () => {
                                        setFormError(null);
                                        setProfessionRequestsCharacterId(null);
                                    }, children: "Cerrar" })] }), formError ? _jsx("p", { className: "error-text", children: formError }) : null, _jsxs("div", { className: "profession-request-list", children: [selectedProfessionRequests.map((request) => (_jsxs("article", { className: "profession-request-card", children: [_jsxs("div", { children: [_jsx("strong", { children: request.professionName }), _jsx("span", { children: request.ownerEmail }), _jsxs("small", { children: ["Solicitada: ", request.requestedAt ? formatDate(request.requestedAt) : "Sin fecha"] })] }), _jsx("div", { className: "profession-requirement-list", children: request.eligibility.requirementResults.map((requirement) => (_jsxs("span", { className: requirement.met ? "is-met" : "is-pending", children: [requirement.met ? "✓" : "○", " ", requirement.label, requirement.hasMaster ? " · Maestro" : ""] }, requirement.id))) }), _jsxs("div", { className: "toolbar", children: [_jsx("button", { type: "button", className: "subtle-button", disabled: isSaving, onClick: () => void handleProfessionDecision(request.id, "reject"), children: "Rechazar" }), _jsx("button", { type: "button", disabled: isSaving || !request.eligibility.eligible, onClick: () => void handleProfessionDecision(request.id, "approve"), children: "Aprobar" })] })] }, request.id))), selectedProfessionRequests.length === 0 ? _jsx("p", { className: "section-help", children: "Este personaje no tiene solicitudes pendientes." }) : null] })] }) })) : null, isDirector && isInviteMemberModalOpen ? (_jsx("section", { className: "modal-backdrop", onClick: () => {
                    if (!isSaving) {
                        setFormError(null);
                        setMemberEmail("");
                        setIsInviteMemberModalOpen(false);
                    }
                }, children: _jsxs("div", { className: "panel modal-panel campaign-link-character-modal campaign-invite-member-modal", role: "dialog", "aria-modal": "true", "aria-labelledby": "campaign-invite-member-title", onClick: (event) => event.stopPropagation(), children: [_jsxs("div", { className: "row-actions campaign-link-character-modal-header", children: [_jsxs("div", { children: [_jsx("h3", { id: "campaign-invite-member-title", children: "Invitar jugador" }), _jsx("p", { className: "section-help", children: "Enviaremos un email para que el jugador pueda aceptar su incorporaci\u00F3n a la campa\u00F1a." })] }), _jsx("button", { type: "button", className: "subtle-button", disabled: isSaving, onClick: () => {
                                        setFormError(null);
                                        setMemberEmail("");
                                        setIsInviteMemberModalOpen(false);
                                    }, children: "Cerrar" })] }), formError ? _jsx("p", { className: "error-text", children: formError }) : null, _jsxs("form", { className: "campaign-link-character-modal-form", onSubmit: (event) => {
                                event.preventDefault();
                                void handleSendInvitation();
                            }, children: [_jsxs("label", { className: "field", children: [_jsx("span", { children: "Email del jugador" }), _jsx("input", { type: "email", autoComplete: "email", autoFocus: true, required: true, value: memberEmail, onChange: (event) => setMemberEmail(event.target.value), disabled: isSaving })] }), _jsxs("div", { className: "toolbar campaign-link-character-modal-actions", children: [_jsx("button", { type: "button", className: "subtle-button", disabled: isSaving, onClick: () => {
                                                setFormError(null);
                                                setMemberEmail("");
                                                setIsInviteMemberModalOpen(false);
                                            }, children: "Cancelar" }), _jsx("button", { type: "submit", disabled: isSaving || !memberEmail.trim(), children: isSaving ? "Enviando..." : "Enviar invitación" })] })] })] }) })) : null, isDirector && isLinkCharacterModalOpen ? (_jsx("section", { className: "modal-backdrop", onClick: () => {
                    if (!isSaving) {
                        setFormError(null);
                        setIsLinkCharacterModalOpen(false);
                    }
                }, children: _jsxs("div", { className: "panel modal-panel campaign-link-character-modal", role: "dialog", "aria-modal": "true", "aria-labelledby": "campaign-link-character-title", onClick: (event) => event.stopPropagation(), children: [_jsxs("div", { className: "row-actions campaign-link-character-modal-header", children: [_jsxs("div", { children: [_jsx("h3", { id: "campaign-link-character-title", children: "Solicitar vinculaci\u00F3n" }), _jsx("p", { className: "section-help", children: "El propietario recibir\u00E1 un email y el personaje solo se a\u00F1adir\u00E1 cuando acepte la solicitud." })] }), _jsx("button", { type: "button", className: "subtle-button", disabled: isSaving, onClick: () => {
                                        setFormError(null);
                                        setIsLinkCharacterModalOpen(false);
                                    }, children: "Cerrar" })] }), formError ? _jsx("p", { className: "error-text", children: formError }) : null, _jsxs("div", { className: "campaign-link-character-modal-form", children: [_jsxs("label", { className: "field", children: [_jsx("span", { children: "Personaje disponible" }), _jsxs("select", { value: selectedAvailableCharacterId, onChange: (event) => setSelectedAvailableCharacterId(event.target.value), disabled: isSaving || linkableCharacters.length === 0, children: [linkableCharacters.length === 0 ? _jsx("option", { value: "", children: "Sin personajes disponibles" }) : null, linkableCharacters.map((entry) => (_jsxs("option", { value: entry.characterId, children: [entry.name, " - ", entry.ownerEmail] }, entry.characterId)))] })] }), linkableCharacters.length === 0 ? (_jsx("p", { className: "section-help", children: "Todos los personajes disponibles ya est\u00E1n vinculados o tienen una solicitud pendiente." })) : null, (selectedCampaign?.pendingCharacterLinkRequests ?? []).length > 0 ? (_jsxs("div", { className: "campaign-pending-invitations", children: [_jsx("h4", { children: "Solicitudes pendientes" }), _jsx("div", { className: "campaign-invite-grid", children: (selectedCampaign?.pendingCharacterLinkRequests ?? []).map((request) => (_jsxs("article", { className: "campaign-invite-item", children: [_jsxs("div", { className: "campaign-invite-copy", children: [_jsx("strong", { children: request.characterName }), _jsx("span", { children: request.ownerEmail }), _jsxs("span", { children: ["Enviada: ", formatDate(request.createdAt)] })] }), _jsx("button", { type: "button", className: "subtle-button", disabled: isSaving, onClick: () => void handleDismissCharacterLinkRequest(request.id, request.campaignId), children: "Cancelar solicitud" })] }, request.id))) })] })) : null] }), _jsxs("div", { className: "toolbar campaign-link-character-modal-actions", children: [_jsx("button", { type: "button", className: "subtle-button", disabled: isSaving, onClick: () => {
                                        setFormError(null);
                                        setIsLinkCharacterModalOpen(false);
                                    }, children: "Cancelar" }), _jsx("button", { type: "button", disabled: isSaving || !selectedAvailableCharacterId, onClick: () => void handleRequestCharacterLink(), children: isSaving ? "Enviando..." : "Enviar solicitud" })] })] }) })) : null, isDirector && isBurdenSummaryModalOpen ? (_jsx("section", { className: "modal-backdrop", onClick: () => {
