...
const POWER_POOL_BY_ARCHETYPE: Record<(typeof SYMBAROUM_ARCHETYPES)[number], string[]> = {
...
type RatedEntry = CreateCharacterInput["sheet"]["habilidades"][number];
...
function generateStartingCapabilities(
  ...
  const habilidades: RatedEntry[] = [];
  const poderesMisticos: RatedEntry[] = [];
...
function buildLevels(noviceCount: number, adeptCount: number): SkillLevel[] {
...
function buildRatedEntry(
  name: string,
  level: SkillLevel,
  tipo: "Habilidad" | "Poder místico",
  fromCatalog:
    | (typeof SYMBAROUM_ABILITIES)[number]
    | (typeof SYMBAROUM_MYSTIC_POWERS)[number]
    | undefined
): RatedEntry {
  return {
    nombre: name,
...
function selectMysticBaseAbility(
  archetype: (typeof SYMBAROUM_ARCHETYPES)[number],
  selectedAbilities: string[]
): string {
  const existingBase = selectedAbilities.find((entry) => MYSTIC_BASE_ABILITIES.includes(entry as (typeof MYSTIC_BASE_ABILITIES)[number]));
  if (existingBase) {
    return existingBase;
  }

  const preferredPool = ABILITY_POOL_BY_ARCHETYPE[archetype].filter((entry) =>
    MYSTIC_BASE_ABILITIES.includes(entry as (typeof MYSTIC_BASE_ABILITIES)[number])
  );
