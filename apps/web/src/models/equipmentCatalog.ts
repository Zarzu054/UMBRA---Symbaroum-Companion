export type EquipmentCatalogGroup = "elixir" | "minor-artifact" | "trap" | "tool" | "equipment";

export type EquipmentReference = {
  source: "Libro Básico" | "Guía Avanzada del Jugador";
  page: number;
};
...
export type EquipmentVariantDefinition = {
  id: string;
  label: string;
  price: string;
  effect?: string;
...
export type EquipmentCatalogDefinition = {
  id: string;
  ...
  detail: string;
  price?: string;
  category: "gear" | "consumable" | "artifact" | "other";
  stackable?: boolean;
  slot?: "artifact" | "worn" | "none";
  qualities?: string[];
  defaultQuantity?: number;
  usable?: boolean;
  facts?: EquipmentFact[];
  variants?: EquipmentVariantDefinition[];
  references: EquipmentReference[];
  tags?: string[];
};

const BASIC_ELIXIR_REFERENCE: EquipmentReference = { source: "Libro Básico", page: 151 };
const ADVANCED_ELIXIR_REFERENCE: EquipmentReference = { source: "Guía Avanzada del Jugador", page: 120 };
const MINOR_ARTIFACT_REFERENCE: EquipmentReference = { source: "Guía Avanzada del Jugador", page: 123 };
const TRAP_REFERENCE: EquipmentReference = { source: "Guía Avanzada del Jugador", page: 127 };
const TOOL_REFERENCE: EquipmentReference = { source: "Guía Avanzada del Jugador", page: 128 };
const BASIC_EQUIPMENT_REFERENCE: EquipmentReference = { source: "Libro Básico", page: 152 };
const BASIC_TOOL_REFERENCE: EquipmentReference = { source: "Libro Básico", page: 153 };
...
function elixir(
  ...
  summary: string,
  detail: string,
  references: EquipmentReference[] = [ADVANCED_ELIXIR_REFERENCE],
  extra: Partial<EquipmentCatalogDefinition> = {}
...
const ELIXIR_DEFINITIONS: EquipmentCatalogDefinition[] = [
...
function artifact(id: string, name: string, price: string, summary: string, detail: string, tier: "Principiante" | "Adepto" | "Maestro", extra: Partial<EquipmentCatalogDefinition> = {}): EquipmentCatalogDefinition {
  return {
...
const MINOR_ARTIFACT_DEFINITIONS: EquipmentCatalogDefinition[] = [
...
const TRAP_DEFINITIONS: EquipmentCatalogDefinition[] = [
  {
...
function tool(id: string, name: string, summary: string, detail: string, extra: Partial<EquipmentCatalogDefinition> = {}): EquipmentCatalogDefinition {
  return {
...
const ADVANCED_TOOL_DEFINITIONS: EquipmentCatalogDefinition[] = [
...
type SimpleEquipmentRow = readonly [id: string, name: string, price: string, kind?: "tool" | "container" | "ammo"];

const BASIC_EQUIPMENT_ROWS: SimpleEquipmentRow[] = [
...
const BASIC_EQUIPMENT_DEFINITIONS: EquipmentCatalogDefinition[] = BASIC_EQUIPMENT_ROWS.map(([id, name, price, kind]) => ({
...
export const EQUIPMENT_CATALOG_DEFINITIONS: EquipmentCatalogDefinition[] = [
  ...ELIXIR_DEFINITIONS,
...
export function getEquipmentDefinitionInventoryVariants(definition: EquipmentCatalogDefinition): Array<{
  templateId: string;
  name: string;
  price: string;
  description: string;
  variant?: EquipmentVariantDefinition;
}> {
