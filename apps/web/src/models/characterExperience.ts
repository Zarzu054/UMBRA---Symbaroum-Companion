...
export type CharacterExperienceSummary = {
  capabilityExpenses: Array<{
    name: string;
    kind: CharacterSheet["capabilitySelections"][number]["kind"];
    level?: string;
    cost: number;
  }>;
...
export function getCharacterExperienceSummary(
  ...
  const spentFromLegacyRituals = sheet.rituales.reduce(
    (total, entry) => {
  ...
  const spentFromLegacyBlessings = sheet.bendiciones.reduce(
    (total, entry) => {
