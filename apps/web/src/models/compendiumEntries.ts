...
export type RuleCategory = "core" | "official_optional" | "homebrew";
...
export const RULE_CATEGORY_DESCRIPTIONS: Record<RuleCategory, string> = {
...
export type CompendiumSummaryReference = { document: "rules"; section: string };
...
export type CompendiumEntry = {
  ...
  summaryReference?: CompendiumSummaryReference;
  media?: Array<{
    src: string;
    alt: string;
    caption?: string;
...
export type RuleCompendiumEntry = CompendiumEntry & {
  tipo: "regla";
  ruleCategory: RuleCategory;
};
...
const BASIC_SUMMARY_RULE_PAGES: Record<number, number> = {
  2: 160,
  3: 159,
  4: 157,
  5: 159,
...
const CORE_OPTIONAL_RULE_PAGES: Record<number, number> = {
  22: 176,
  23: 176,
  24: 177,
  25: 177,
...
const ADVANCED_OPTIONAL_RULE_PAGES: Record<number, number> = {
...
const RULE_CONTENT_OVERRIDES: Record<string, Pick<RuleMetadata, "summary" | "detail">> = {
...
const RULE_PARENT_BY_ID: Record<string, string> = {
...
function getLegacyRuleMetadata(entry: CompendiumEntry): RuleMetadata {
...
function normalizeRuleEntry(entry: CompendiumEntry): RuleCompendiumEntry {
...
const ADDITIONAL_RULE_ENTRIES: RuleCompendiumEntry[] = [
  {
...
const RULE_SECTION_OVERRIDES: Record<string, Partial<CompendiumVariant>> = {
...
const LOCALIZED_HIT_VARIANT: CompendiumVariant = {
...
const RULE_EXTRA_VARIANTS_BY_PARENT: Readonly<Record<string, CompendiumVariant[]>> = {
...
function consolidateRuleFamilies(entries: RuleCompendiumEntry[]): RuleCompendiumEntry[] {
  const byId = new Map(entries.map((entry) => [entry.id, entry]));
  const childrenByParent = new Map<string, RuleCompendiumEntry[]>();
  ...
  return entries
    ...
    .map((entry) => {
      ...
      const childVariants = children.map((child): CompendiumVariant => {
...
export const CORE_RULES: RuleCompendiumEntry[] = consolidateRuleFamilies([
...
).filter((entry) => !["robusto", "cambiaformas", "longevo"].includes(normalizeLookup(entry.nombre)));
...
export const SYMBAROUM_CHARACTER_TRAITS: CompendiumEntry[] = [
  {
...
export const COMPENDIUM_ENTRY_ID_ALIASES: Readonly<Record<string, string>> = Object.freeze(
  Object.fromEntries(ALL_ENTRIES.flatMap((entry) => (entry.legacyIds ?? []).map((legacyId) => [legacyId, entry.id])))
);

export function findCompendiumEntryById(entryId: string): CompendiumEntry | null {
