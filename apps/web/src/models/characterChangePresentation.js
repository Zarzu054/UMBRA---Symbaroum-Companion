...
function collectionKey(path) {
    const normalized = rootPath(path);
    const match = normalized.match(/^([^.[\]]+)\[([^\]]+)\]/);
    return match ? `${match[1]}[${match[2]}]` : null;
}
function collectionIdentifier(path) {
...
function normalizedValue(value) {
    ...
    if (isRecord(value))
        return Object.fromEntries(Object.entries(value).map(([key, entry]) => [key, normalizedValue(entry)]));
    return value;
}
...
function itemName(changes, identifier) {
    ...
    for (const change of changes) {
        if (["name", "nombre", "label", "title", "titulo"].includes(lastField(change.path))) {
            ...
            if (scalar)
                return scalar.replace(/\s*\([^)]*\)\s*$/u, "");
        }
    }
    return humanIdentifier(identifier);
}
function humanField(path, fallback) {
    const key = collectionKey(path);
    if (key && rootPath(path) === key)
        return "Contenido";
    const field = lastField(path);
    if (FIELD_LABELS[field])
        return FIELD_LABELS[field];
    if (fallback && !/^(label|notes|effect summary|condition[-_])/iu.test(fallback))
        return fallback;
    return field
...
function presentSingle(change, index) {
...
export function presentCharacterChanges(changes) {
