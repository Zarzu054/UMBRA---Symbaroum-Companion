...
function waitForBridgeResponse(requestId, timeoutMs = 1500) {
    return new Promise((resolve) => {
        const timer = window.setTimeout(() => {
            window.removeEventListener(ROLL20_BRIDGE_RESPONSE_EVENT, handleResponse);
            resolve({
        ...
        window.addEventListener(ROLL20_BRIDGE_RESPONSE_EVENT, handleResponse);
    });
}
export function getRollDestination() {
    const raw = window.localStorage.getItem(STORAGE_KEY);
    return raw === "umbra" || raw === "roll20" || raw === "both" ? raw : "roll20";
}
export function setRollDestination(destination) {
    window.localStorage.setItem(STORAGE_KEY, destination);
}
export async function pingRoll20Bridge() {
...
export async function dispatchRoll20Request(request, visibility = "public") {
