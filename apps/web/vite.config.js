...
const PDFJS_WASM_ASSETS = ["openjpeg.wasm", "openjpeg_nowasm_fallback.js", "qcms_bg.wasm"];
    ...
    return {
        ...
        configureServer(server) {
            server.middlewares.use(PDFJS_WASM_PUBLIC_PATH, (request, response, next) => {
                ...
                if (!PDFJS_WASM_ASSETS.includes(fileName)) {
                    next();
                    return;
                }
                response.statusCode = 200;
                response.setHeader("Content-Type", fileName.endsWith(".wasm") ? "application/wasm" : "application/javascript; charset=utf-8");
